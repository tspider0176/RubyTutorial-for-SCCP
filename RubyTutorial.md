...
__オブジェクト指向言語__  
今習っているC言語は手続き型言語と呼ばれており、
こちらは"操作手順"に重点を置いたプログラミングを目的として設計されている。それに対してオブジェクト指向型言語とは、
"操作対象"に重点を置いて設計を行うプログラミング言語である。  
...
前ではREPLを紹介したが、本節では実際にAOJの問題を解く時など比較的大きなプログラムを組むことになる場合に備えて、ソースコードをファイルに記述した場合の実行方法を教える。適当なテキストエディタを開き、実際にRubyのHello Worldを書いてみよう。
