...
資料URL: https://github.com/tspider0176/RubyTutorial-for-SCCP/blob/master/ruby.md

## 1 はじめに
rubyには対話型Rubyシェルという物が備わっていて、REPL(Read-eval-print loop, れぷる)と呼ばれている。英単語から分かる通り、読んで(read)、評価して(eval)、表示して(print)、繰り返す(loop)物であり、コードをシェルに打ち込むとそれを評価して表示してくれる代物である。動作・仕様が知りたい関数の挙動を確かめたり、自分のコードが正しく動くかを確かめる時などに重宝する。
実行はターミナル上にて

    $ irb
と打ち込む事で起動出来る。下のような文字がコンソール上に表示されたら起動した証拠だ。
...
と打ち込んでみよう。
    ...
    => 2
と結果が返ってくるはずだ。これだけではただの計算機だが、rubyの基本構文等は全てこのREPL上で実行、表示する事が出来る。もちろんテキストファイルを作成しコードを記述してコンパイル、と言った手順でも動作確認が可能であり、以下のように実行できる。

    # コードが書かれたファイル test.rb をコンパイル
    $ ruby test.rb  
しかし、今後の章で紹介するコードは殆ど小規模な物なのでREPL上で動作を確認した方が手軽で良いだろう。終了するにはCtrl-Dを入力するか、exitまたはquitとキーボードから入力すれば良い。

## 2 制御式

### 2.1 式とは？
C言語の制御「文」とは違い、rubyでは制御「式」と言う呼び方が使われている。これらの文と式の違いは何だろうか？答えは、式の値を返すかどうかということだ。これだけ言われてもピンとこないかもしれないが、今まで習ってきたC言語の授業を思い出してみて欲しい。あの授業では、if文、switch-case文、while文、for文、全て式を処理させるものだったが、rubyでは基本構文そのものを値として使うことが可能になっている。
...
ここでは今までのprintに変わって新しくputsという関数を使っているが、これは文末に自動的に改行を入れてくれる関数である。  
ここまでの内容を用いて、for文の確認としては以下のAOJの問題が解ければ大丈夫だろう。  
